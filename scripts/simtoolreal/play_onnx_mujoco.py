...
This is the first reproduction milestone for porting SimToolReal into MJLab:
it mirrors the browser demo's MuJoCo/ONNX loop while running from MJLab's uv
environment.  The next step is to move this logic behind a manager-based MJLab
environment once the state/action/observation parity is established.
"""
