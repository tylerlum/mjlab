...
def _split_pose7_array(
  poses: np.ndarray, *, name: str
...
def _build_object_trajectories(
  object_poses: dict[str, np.ndarray] | None, *, num_frames: int
) -> dict:
  if not object_poses:
    return {}
  ...
  for object_name, pose_values in object_poses.items():
    positions, quats = _split_pose7_array(
      pose_values, name=f'object_poses["{object_name}"]'
    )
...
def build_trajectory_payload(
  *,
  ...
  """
  joint_positions = _as_2d_float_array(
    robot_joint_positions, name="robot_joint_positions"
  )
  ...
  if timestamps is None:
    ...
    timestamps_array = _as_1d_float_array(
      timestamps, name="timestamps", length=num_frames
    )
  ...
  trajectory = {
    ...
    "object_trajectories": _build_object_trajectories(
      object_poses, num_frames=num_frames
    ),
  }

  if robot_base_poses is not None:
    base_positions, base_quats = _split_pose7_array(
      robot_base_poses, name="robot_base_poses"
    )
  ...
  *,
  ...
  return render_template(
    template_path,
    {
      ...
      "default_show_frame_axes": bool(default_show_frame_axes),
    },
  )
